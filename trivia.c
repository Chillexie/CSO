...
int readTriviaQuestions(char *category, int numQuestions) {
    ...
    // char buffer[4096];//local stack variable 
    char *buffer = malloc(4096); //store it on a heap to be a global variable  


    for (int i = 0; i < numQuestions; i++) {

        // Request enough space for a questionData on the heap
        questionData *question = malloc(sizeof(questionData));   //
...
int main() {
    ...
    fgets(category, 20, stdin); //reads 20 characters from keyboard

    // What happens if they enter something different?  Oops! //continues to next call before exiting 

    // Load the questions -- do these numbers add up?
    int success = readTriviaQuestions(strtok(category, "\n"), numQuestions);

    // Check that we were able to open the file and read the questions -- what is readok again? //possible return value when reading questions 
    if (success != readok) {
        printf("An error occurred when reading the questions. Error code: %d", success);
        return 1;
    }

    printf("Okay, %s, now loading %d questions to get started.\n\n", name, numQuestions); //after category check 
